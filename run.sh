...
for SERVICE in $ML_SERVICES; do
    ...
    if [ -z "$MODEL_TYPE" ] || [ -z "$ROUTER_TYPE" ]; then
        case "$SERVICE" in
            ...
            *rb2*|*deep-neural*)
                ...
                ;;
            *)
                echo "  ??  Could not determine model configuration for $SERVICE"
                echo ""
                continue
                ;;
        esac
    ...
    if [ -f "$SCALER_FILE" ]; then
        echo "  ? Scaler file found: $SCALER_FILE"
    else
        echo "  ??  Scaler file not found: $SCALER_FILE (may not be required for this model)"
    fi
    echo ""
done
...
if [ $? -eq 0 ]; then
  ...
  if [ $SERVICE_COUNT -eq $TOTAL_SERVICES ]; then
    echo "?? All ML inference services are running correctly!"
  else
    echo "??  Some services failed to start. Check the logs for more information."
  fi
