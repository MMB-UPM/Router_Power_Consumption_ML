...
def parse_args():
    ...
    parser.add_argument('--experiment', type=str, default="all", 
                        help='Specific experiment to use (e.g., ExperimentoA1) or "all" for sequential replay')
...
class TelemetryMock:
    ...
    def _load_test_data(self):
        ...
        def sort_key(file_path):
            ...
            # Extract experiment ID (A1, A3, A4)
            experiment_part = filename.split('_')[0]  # "ExperimentoA1"
            if experiment_part.startswith("Experimento"):
                exp_id = experiment_part[11:]  # Remove "Experimento" prefix to get "A1"
            else:
                exp_id = experiment_part
            
            # Define experiment order
            exp_order = {'A1': 1, 'A3': 2, 'A4': 3}
    ...
    def _update_current_experiment(self, file_path):
        ...
        if parts:
            # Extract experiment ID from filename like "ExperimentoA1_test_data.csv" -> "A1"
            experiment_part = parts[0]  # "ExperimentoA1"
            if experiment_part.startswith("Experimento"):
                self.current_experiment = experiment_part[11:]  # Remove "Experimento" prefix to get "A1"
            else:
    ...
    def _get_next_test_record(self):
        ...
        if self.current_record_index < len(self.test_data_records):
            ...
            if hasattr(self, 'current_file_path') and self.current_file_path:
                ...
                # Extract experiment name (assuming format like "ExperimentoAX_*.csv")
                parts = filename.split('_')
